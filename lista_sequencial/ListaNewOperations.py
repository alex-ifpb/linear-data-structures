...
'''
O Arquivo ListaPlus.py corresponde ao arquivo Lista.py incrementado de questões
apresentadas em listas de exercicios ou provas
'''        
class Lista:
    ...
    '''
    Prova Final 2018.2 RC
    '''
    def getSubtracaoOrdemInversa(self):
        '''
        if( self.estaVazia()):
            return 0
        else:
            subtracao = self.elemento(self.tamanho())
            for i in range(self.tamanho()-1,0,-1):
                print(self.elemento(i))
                subtracao -= self.elemento(i)
                
        return subtracao
        '''
        aux = list(reversed(self.__dado))
        subtracao = None
        for num in aux:
            if subtracao == None:
                subtracao = num
...
try:
    ...
    print(lst.getSubtracaoOrdemInversa())
